...
RSpec.describe MaintenanceSchedule, type: :model do
  ...
  def build_schedule(attrs = {})
    MaintenanceSchedule.new(
      { iceberg_table: table, operation: "optimize", cron: "0 3 * * *" }.merge(attrs)
    )
  end

  describe "config validation" do
    it "accepts blank config values submitted by the form" do
      schedule = build_schedule(operation: "expire_snapshots",
  ...
  describe "cron validation" do
    it "accepts a valid cron expression" do
      expect(build_schedule(cron: "0 3 * * *")).to be_valid
    end

    it "rejects a cron with out-of-range fields" do
      schedule = build_schedule(cron: "0 99 99 99 99")
    ...
    it "rejects a non-cron string" do
      expect(build_schedule(cron: "not a cron")).not_to be_valid
    end
  end
end
