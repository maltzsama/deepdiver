...
Common labels.
*/}}
{{- define "deepdiver.labels" -}}
helm.sh/chart: {{ .Chart.Name }}-{{ .Chart.Version | replace "+" "_" }}
app.kubernetes.io/name: {{ include "deepdiver.name" . }}
...
*/}}
{{- define "deepdiver.workerSelector" -}}
app.kubernetes.io/name: {{ include "deepdiver.name" . }}
app.kubernetes.io/instance: {{ .Release.Name }}
...
*/}}
{{- define "deepdiver.workerFreshnessSelector" -}}
app.kubernetes.io/name: {{ include "deepdiver.name" . }}
app.kubernetes.io/instance: {{ .Release.Name }}
...
*/}}
{{- define "deepdiver.workerEngineSelector" -}}
app.kubernetes.io/name: {{ include "deepdiver.name" . }}
app.kubernetes.io/instance: {{ .Release.Name }}
...
of db:prepare.
*/}}
{{- define "deepdiver.migrateSelector" -}}
app.kubernetes.io/name: {{ include "deepdiver.name" . }}
app.kubernetes.io/instance: {{ .Release.Name }}
...
*/}}
{{- define "deepdiver.trinoSecretName" -}}
{{- printf "%s-trino-catalog-secrets" (include "deepdiver.fullname" .) }}
{{- end }}
...
broken login.
*/}}
{{- define "deepdiver.hasChartEnvSecret" -}}
...
*/}}
{{- define "deepdiver.envFrom" -}}
- configMapRef:
    name: {{ include "deepdiver.fullname" . }}
{{- if .Values.appSecrets.existingSecret }}
- secretRef:
    name: {{ .Values.appSecrets.existingSecret | quote }}
{{- else if .Values.appSecrets.create }}
- secretRef:
    name: {{ include "deepdiver.fullname" . }}-app
{{- end }}
{{- if include "deepdiver.hasChartEnvSecret" . }}
- secretRef:
    name: {{ include "deepdiver.fullname" . }}-chart-env
{{- end }}
{{- end }}
...
*/}}
{{- define "deepdiver.trinoSecretRules" -}}
  ...
  - apiGroups: [ "" ]
    resources: [ "secrets" ]
    resourceNames: [ {{ include "deepdiver.trinoSecretName" . | quote }} ]
...
*/}}
{{- define "deepdiver.trinoDeploymentRules" -}}
...
*/}}
{{- define "deepdiver.trinoRules" -}}
rules:
{{- if ne .Values.trino.provisioner "fake" }}
{{ include "deepdiver.trinoDeploymentRules" . }}
{{- end }}
{{ include "deepdiver.trinoSecretRules" . }}
{{- end }}
...
*/}}
{{- define "deepdiver.podAnnotations" -}}
{{- $root := .root }}
{{- $computed := dict "checksum/config" (include (print $root.Template.BasePath "/configmap.yaml") $root | sha256sum) }}
{{- if or $root.Values.appSecrets.create (include "deepdiver.hasChartEnvSecret" $root) }}
...
*/}}
{{- define "deepdiver.workerDeployment" -}}
{{- $root := .root }}
...
kind: Deployment
metadata:
  name: {{ include "deepdiver.fullname" $root }}-{{ .component }}
  labels:
    {{- include "deepdiver.labels" $root | nindent 4 }}
...
spec:
  ...
  template:
    metadata:
      annotations:
        {{- include "deepdiver.podAnnotations" (dict "root" $root) | nindent 8 }}
      labels:
        ...
        {{- end }}
    spec:
      serviceAccountName: {{ include "deepdiver.fullname" $root }}
      ...
      containers:
        - name: solid-queue
          ...
          env:
            ...
            {{- end }}
          envFrom:
            {{- include "deepdiver.envFrom" $root | nindent 12 }}
          volumeMounts:
            {{- if and $root.Values.internalCA.enabled $root.Values.internalCA.existingSecret }}
            - name: internal-ca
